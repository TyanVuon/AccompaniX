...
class DeepBach:
    ...
    def load_models(self, search_params=None):
        if search_params:
            ...
            for voice_index in range(self.num_voices):
                pattern = f"model_*_vi{voice_index}_*.pt"
                voice_files = glob.glob(os.path.join('models', pattern))
                if voice_files:
                    model_file = voice_files[0]
                    print(f"Loading model for voice {voice_index} from file: {model_file}")

                    # Initialize the VoiceModel with the instance variables
                    ...
                    print(f"No model file found for voice {voice_index}")
...
# deepbach_model.load_models(search_params=search_params)
    ...
    def save(self, details=None):
        for voice_index, voice_model in enumerate(self.voice_models):
            # Modify details to include voice index
            voice_details = {**details, 'voice_index': voice_index} if details is not None else None
            voice_model.save(voice_details)
